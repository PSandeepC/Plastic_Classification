...
if page == "Home":
    ...
    if uploaded_file is not None:
        ...
        bars = ax.bar(CLASS_NAMES, predictions, color='#4CAF50')  # Use a consistent color
        ax.set_xlabel("Plastic Type", fontsize=12)  # Adjust font size
        ax.set_ylabel("Probability", fontsize=12)  # Adjust font size
        ax.set_title("Prediction Probabilities", fontsize=14)  # Adjust font size
        ax.set_xticklabels(CLASS_NAMES, rotation=45, ha='right', fontsize=10)  # Rotate and adjust font size of x-axis labels
    ...
    **Data Augmentation** was applied to the training set to improve the model's ability to generalize:
    ...
    st.markdown("""
    ## Conclusion
    The model achieved an accuracy of **92.03%** after training. This indicates that the model is able to accurately classify plastic types most of the time, although there is still room for improvement.
...
st.markdown(
    ...
    **Plastic Classification App**  
    ...
    Model trained to classify plastic types: HDPE, LDPE, Other, PET, PP, PS, PVC, NON PLASTIC.
    """
)
